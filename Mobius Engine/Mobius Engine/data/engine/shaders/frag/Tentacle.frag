...
{		
    ...
    vec2 newUV = fs_in.TexUV + vec2(sin(fs_in.TexUV.y * 2 + fs_in.TexUV.x * 6 + uTime * 1.4 + h * h * 2), cos(fs_in.TexUV.x * 3 + fs_in.TexUV.y * 6 + uTime * 0.4 + h * h * 1.5)) * h * 0.06;
    ...
    vec3 _WorldSpaceLightPos0 = vec3(0.0, 1.0, 2.0);
    float ndotl = clamp(dot(fs_in.Normal, _WorldSpaceLightPos0), 0.0, 1.0) * 0.5 + 0.5;
    ...
    vec3 bodyColor = vec3(0.15, 0.0, 0.2);
