...
{		
    ...
    if (fs_in.ShellLayer >= 0)
    {
        ambientOcclusion = pow(h, uOcclusionAtt);
        ambientOcclusion += uOcclusionBias;
        ambientOcclusion = clamp(ambientOcclusion, 0.0, 1.0);
    }
        
    
    vec3 texCol = texture(uTexture, vec2(fs_in.TexUV.x, -fs_in.TexUV.y)).rgb;
    FragColor = vec4(texCol * uShellColor * ndotl * ambientOcclusion, !outsideThickness || fs_in.ShellLayer == 0);
}