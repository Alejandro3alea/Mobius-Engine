...
bool LoadWavFileHeader(std::ifstream& file, int& channels, int& sampleRate, int& bitsPerSample, int& size)
{
    char buffer[4];
    if (!file.is_open())
        return false;
...
Sound::Sound(const std::string& file)
{
    ...
    char* data = LoadWAV(file, channels, sampleRate, bitsPerSample, size);
    if (!data)
    {
        std::cerr
    ...
    mNumChannels = static_cast<unsigned>(channels);
    ...
    alBufferData(mBufferID, format, data, size, sampleRate);
}
