...
// frame_padding = 0: no framing
// frame_padding > 0: set framing size
bool ImGui::ImageButton(const char* id, ImTextureID user_texture_id, const ImVec2& size, const ImVec2& uv0, const ImVec2& uv1, int frame_padding, const ImVec4& bg_col, const ImVec4& tint_col)
{
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = g.CurrentWindow;
    if (window->SkipItems)
    ...
    // Default to using texture ID as ID. User can still push string/integer prefixes.
    const ImGuiID strID = window->GetID(id);

    const ImVec2 padding = (frame_padding >= 0) ? ImVec2((float)frame_padding, (float)frame_padding) : g.Style.FramePadding;
    return ImageButtonEx(strID, user_texture_id, size, uv0, uv1, padding, bg_col, tint_col);
}
...
// FIXME: See references to #2304 in .txt
